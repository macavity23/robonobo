...
public class DbService extends AbstractService {
	...
	public static final String CREATE_SEEN_COMMENTS_TBL = "CREATE CACHED TABLE seen_comments (comment_id BIGINT NOT NULL)";
	...
	private static final String READ_SEEN_COMMENT = "SELECT * FROM seen_comments WHERE comment_id = ?";
	...
	private static final String CREATE_SEEN_COMMENT = "INSERT INTO seen_comments (comment_id) VALUES(?)";
	...
	public boolean haveSeenComment(long commentId) {
		if (!running)
			return false;
		Connection conn = null;
		try {
			conn = getConnection();
			PreparedStatement ps = conn.prepareStatement(READ_SEEN_COMMENT);
	...
	public void markAllCommentsAsSeen(Collection<Comment> comments) {
		...
		try {
			conn = getConnection();
			PreparedStatement ps = conn.prepareStatement(CREATE_SEEN_COMMENT);
			...
			log.error("Error marking comments as seen", e);
		} finally {
			if (conn != null)
				returnConnection(conn);
		}
