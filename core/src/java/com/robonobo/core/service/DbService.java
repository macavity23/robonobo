...
public class DbService extends AbstractService {
	...
	private static final String CREATE_PLAYLIST_SEEN_SIDS_TBL = "CREATE CACHED TABLE playlist_seen_sids (playlist_id_str VARCHAR(64) NOT NULL, stream_id VARCHAR(36) NOT NULL)";
	...
	private static final String READ_PLAYLIST_SEEN_SIDS = "SELECT stream_id FROM playlist_seen_sids WHERE playlist_id_str = ?";
	...
	private static final String READ_LIBRARY_NUM_UNSEEN_SIDS = "SELECT COUNT(*) FROM ((SELECT lt.stream_id FROM library_tracks as lt WHERE lt.user_id = ? INTERSECT SELECT s.stream_id FROM streams AS s) MINUS SELECT pss.stream_id FROM playlist_seen_sids AS pss WHERE pss.playlist_id_str = CONCAT('library:', ?))";
	...
	private static final String CREATE_PLAYLIST_SEEN_SID = "MERGE INTO playlist_seen_sids AS pss USING (VALUES(CAST(? AS VARCHAR(64)))) AS vals(x) ON pss.playlist_id_str = vals.x WHEN NOT MATCHED THEN INSERT VALUES(vals.x, CAST(? AS VARCHAR(64)))";
