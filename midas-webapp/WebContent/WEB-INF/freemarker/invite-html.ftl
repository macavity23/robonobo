...
<p>${fromUser.friendlyName} (<a href="mailto:${fromUser.email}" style="color:#5b0d01">${fromUser.email}</a>) has invited you to robonobo, the social music app.<#if playlist??> To welcome you, they have sent you a playlist titled '${playlist.title}' with ${playlist.streamIds?size} tracks.</#if></p>
