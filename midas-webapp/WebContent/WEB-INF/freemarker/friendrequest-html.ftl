...
<p style="color:#111111">${fromUser.friendlyName} (<a href="mailto:${fromUser.email}" style="color:#5b0d01">${fromUser.email}</a>) wants to be your friend on robonobo.<#if playlist??> To welcome you, they have sent you a playlist titled '${playlist.title}' with ${playlist.streamIds?size} tracks.</#if></p>
